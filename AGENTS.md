# FauxCam — Engineering Baseline

Always-on engineering bias for this repository. Primary rule set: **Clean Architecture (Robert C. Martin)**, chosen because FauxCam's spine is a layered, framework-independent design (domain → application → adapters → delivery) with SOLID enforced at the boundaries.

On-demand depth lives in `.claude/skills/` (clean-code, refactoring, a-philosophy-of-software-design, domain-driven-design) and activates per kind of work. Full Clean Architecture reference: `docs/agent-rules/clean-architecture.full.md`. Source + license: `docs/agent-rules/`.
