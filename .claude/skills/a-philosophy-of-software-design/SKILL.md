...
description: Use for module and API design in FauxCam - decomposition, interface shape, reducing complexity, and choosing deep modules over shallow wrappers/pass-through layers. Apply when a change feels awkward or spreads complexity across files (e.g. designing FrameSource/FrameTransport ports, the AVSwizzle surface, or the wire protocol). Derived from "A Philosophy of Software Design" (John Ousterhout). Full reference in reference.md.
---
