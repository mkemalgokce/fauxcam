...
description: Use when modeling FauxCam's domain layer (Frame, Demand, SimDevice, CameraPosition, PixelFormat) and keeping a ubiquitous language across code/tests/docs - when domain behavior and vocabulary drive design more than technical organization. Use to keep persistence, AVFoundation, sockets, and frameworks out of the model. Derived from "Domain-Driven Design" (Eric Evans). Full reference in reference.md.
---
