...
- **Instance-method swizzle**: `AVCaptureDeviceDiscoverySession -devices` returns both fakes. A real discovery session exposes no readable `position`/`deviceTypes`/`mediaType` (they are write-only constructor inputs), so the swizzle cannot filter by the requested position; consumers filter the returned list by each device's reported `position`. Position-specific selection is served instead by `+defaultDeviceWithDeviceType:mediaType:position:`.
